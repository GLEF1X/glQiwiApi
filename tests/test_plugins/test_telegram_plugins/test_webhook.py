...
from glQiwiApi.plugins.telegram.webhook import TelegramWebhookPlugin, DEFAULT_TELEGRAM_WEBHOOK_PATH_PREFIX, \
    DEFAULT_TELEGRAM_WEBHOOK_PATH
from glQiwiApi.utils.certificates import get_or_generate_self_signed_certificate, SSLCertificate
...
async def test_webhook_plugin_install(mocker: MockerFixture, self_signed_certificate: SSLCertificate):
    ...
    plugin = TelegramWebhookPlugin(
        ...
        app_config=ApplicationConfig(
            ssl_certificate=self_signed_certificate
        )
    )
    run_app_mock = mocker.patch('glQiwiApi.plugins.telegram.webhook.run_app', autospec=True)
    await plugin.install(ctx={})

    expected_webhook_url = "localhost" + DEFAULT_TELEGRAM_WEBHOOK_PATH_PREFIX + DEFAULT_TELEGRAM_WEBHOOK_PATH.format(
        token='32423:dfgd'
    )
    spy.assert_awaited_once_with(
        url=expected_webhook_url,
        certificate=self_signed_certificate.as_input_file()
    )
