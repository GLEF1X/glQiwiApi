...
from glQiwiApi.core.builtin import TelegramWebhookProxy
...
async def tg_webhook_proxy_fixture(mocker: MockerFixture):
    ...
    tg_proxy = TelegramWebhookProxy(Dispatcher(bot_mock), webhook_domain="", ssl_certificate=SSLContext(),
                                    loop=EventLoopStub())  # noqa
    yield tg_proxy
