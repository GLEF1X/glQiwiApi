...
class QiwiBillWebhookView(BaseWebhookView[types.BillWebhook]):
    ...
    def _validate_event_signature(self, update: types.BillWebhook) -> None:
        sha256_signature = cast(str, self.request.headers.get("X-Api-Signature-SHA256"))  # pragma: no cover
        ...
        try:  # pragma: no cover
            ...
            logger.debug("Blocking request due to invalid signature of payload.")  # pragma: no cover
