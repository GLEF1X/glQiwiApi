...
def deprecated(reason: Union[str, Callable[..., _F]], stacklevel: int = 2) -> DeprecatedFunc:
    """
    ...
    if inspect.isclass(reason) or inspect.isfunction(reason):
        ...
        def wrapper1(*args: Any, **kwargs: Any) -> _F:
            warn_deprecated(msg1.format(name=func1.__name__), stacklevel=stacklevel)  # type: ignore
            return cast(_F, func1(*args, **kwargs))  # type: ignore
...
def renamed_argument(
    ...
    def decorator(func):
        ...
        if is_coroutine:
            ...
            @functools.wraps(func)
            async def wrapped(*args, **kwargs) -> _F:
...
class DeprecatedReadOnlyClassVar(Generic[_OwnerCls, _VT]):
    ...
    def __get__(self, instance: Optional[_OwnerCls], owner: Type[_OwnerCls]) -> _VT:
