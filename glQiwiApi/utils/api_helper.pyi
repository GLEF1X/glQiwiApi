...
def measure_time(func: types.N) -> types.N: ...
def datetime_to_str_in_iso(
    obj: Union[None, datetime, timedelta], yoo_money_format: bool = False
) -> str: ...
def parse_auth_link(response_data: Dict[Any, Any]) -> Optional[str]: ...
def check_dates(
    start_date: Optional[datetime], end_date: Optional[datetime], payload_data: dict
) -> dict: ...
...
def _await_sync(
    future: asyncio.Future, executor: types.Executors, loop: asyncio.AbstractEventLoop
) -> Any: ...
def _cancel_future(
    loop: asyncio.AbstractEventLoop, future: asyncio.Future, executor: types.Executors
) -> None: ...
...
def async_as_sync(
    func: Callable[..., Coroutine[Any, Any, types.N]]
) -> Callable[..., types.N]: ...
