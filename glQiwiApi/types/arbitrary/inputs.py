...
class AbstractInput(abc.ABC, Generic[InputType]):
    __slots__ = ('_input', '_file_descriptor')
    ...
    def get_path(self) -> str:
        raise TypeError(
            f"{type(self).__qualname__} doesn't provide a mechanism to get path to file"
        )

    def get_filename(self) -> str:
        raise TypeError(f"{type(self).__qualname__} doesn't provide a mechanism to get filename")
